...
VERSION_LIGHTHOUSE=$(printf "${VERSIONS}" | grep lighthouse | cut -f 2 -d ':');
EXISTENCE_TAG="alpine-${VERSION_ALPINE}-chromedriver-${VERSION_CHROMEDRIVER}-lighthouse-${VERSION_LIGHTHOUSE}";
EXISTENCE_REPO_URL="${REPOSITORY_URL}-${EXISTENCE_TAG}";
ALPINE_VERSION_REPO_URL="${REPOSITORY_URL}-${VERSION_ALPINE}-${VERSION_CHROMEDRIVER}-${VERSION_LIGHTHOUSE}";
